...
| `pause.mjs` | pause on a phone: a swipe down pauses and the same swipe resumes, the paused menu really appears, and up is still boost while sideways is still a lane change. Real touch through CDP, so it proves the adapter sees a genuine drag |
